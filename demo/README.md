...
access, then for schools (same pipeline, one argument changed), then the
graph artifact (pixels and roads as a single loadable object).
...
Why Kew Gardens? Nominatim does not have it as an OSM polygon, so we pass
`--bbox` explicitly. Many neighborhoods, informal settlements, and custom
AOIs work this way. The option is documented in the demo so it isn't a
surprise.

Total runtime in the demo: about 2 minutes wall-clock once data is cached.
First-time runs are slower (NASADEM tiles, OSM Overpass, and building
footprints all fetched fresh). Subsequent runs of the same chip complete
in about 20 seconds on an Apple Silicon MacBook.

...
as a 2019 UNICEF Magicbox intern project (Django REST API for distance-to-
healthcare computations). That implementation is preserved on the
[`archive/django-api`](https://github.com/msradam/geospatial-routing-api/tree/archive/django-api)
...
- `notebooks/02_bench_vs_map.py`: reactive marimo notebook for benchmarking
  ...
  cached), clips + reprojects, computes Spearman ρ / MAE / Weiss-bin
...
- Deliberate decision: benchmarking is a research operation, not a CLI
  ...
  use case. Notebook keeps the CLI focused and the validation forkable.

## [v0.5] — 2026-05-17 · CI + lint + tests as shipping signal

- `.github/workflows/test.yml`: GitHub Actions runs pytest + ruff on every
  push and PR. Caches `uv.lock`. No ML extras in CI to keep runs fast.
...
- `tirtha accessibility run --friction fm` now actually invokes the
  foundation model. Fetches Sentinel-2 + Sentinel-1 RTC + NASADEM via
...
- Helpful error message when the `[ml]` extra isn't installed.
- 224×224 fixed input size enforced; raises a clear error with `--bbox` /
  `--resolution` hints if the chip is too small.

## [v0.2] — 2026-05-17 · Graph artifact + Kew Gardens demo

- `src/tirtha/graph.py`: `TirthaGraph` dataclass + `build_graph` /
  `save_graph` / `load_graph` / `to_networkx`. The "image is both a raster
  and a graph" thesis exposed as a first-class loadable object. Bundles a
  unified scipy.sparse CSR adjacency of pixel + OSM-road nodes joined at
  coincident locations, plus per-node coordinates, types, friction, and a
  JSON metadata sidecar — all in a single `.npz`.
- CLI: `tirtha graph build` and `tirtha graph inspect`.
- `--bbox` escape hatch on both `accessibility run` and `graph build`.
  Nominatim doesn't have every neighborhood as a polygon — `--bbox` lets
  users target informal settlements, refugee camps, custom AOIs directly.
...
- Renamed `friction` → `tirtha` everywhere. PyPI-name and CLI entry-point.
- New package layout (flat, 8 modules under `src/tirtha/`):
  - `__init__.py` — public API
  - `pipeline.py` — `run_accessibility()` orchestration
  - `data.py` — Planetary Computer STAC + OSMnx + WorldPop + MAP raster
    loaders, with `raise_for_status` fix to prevent the 0-byte WorldPop
    bug that bit us on the NYC 5-borough run.
...
## Pre-v0.1 — methodology validation (May 16–17, 2026, all on master before refactor)

These are the figures and findings that motivated the v0.1 refactor. See the
git log for full commit messages — each is detailed.
...
- **v4.2** (commit `7b9adb8`) · Split-conformal calibration of the
  uncertainty bounds. Empirical 95% coverage 3% → 94.9%. ECE 0.485 → 0.003.
- **v4.1** (commit `fdb8d94`) · Honest documentation of the *naive*
  uncertainty miscalibration before the conformal fix.
- **v4** (commit `5e357ab`) · Four-way head-to-head: tirtha-rules,
  tirtha-FM, Wu et al. 2025 rule-based, and MAP 2020. At the 10-min walking
  threshold, tirtha-FM agrees with MAP within 1.3 percentage points; Wu's
  rule-based is off by 8.8.
- **v5** (commit `1fdbf55`) · National-scale Sierra Leone run. ~13M pixels
  at 100m, 6.77M population. Tirtha estimates 25.6% of the population is
  more than 3 hours walking from healthcare (1.71M people); MAP 2020
  estimates 0.2%.
- **v6** (commit `dc7d4b3`) · Brownsville, Brooklyn urban application.
  Same pipeline; schools are ~2× as accessible as healthcare in the
  documented health-disparities neighborhood.

## Archive

- `archive/django-api` branch + `v1-unicef-2019-archive` tag preserve the
  2019 UNICEF Magicbox intern implementation (Django REST + OSMnx + iGraph).
  Read-only reference.

...
Adam Munawar Rahman · IBM (personal project) · NYU Tandon MS CompEng ·
Code: [github.com/msradam/geospatial-routing-api](https://github.com/msradam/geospatial-routing-api) ·
Contact: msrahmanadam@gmail.com

---

**TL;DR.** Tirtha uses TerraMind-Small as a *frozen* multimodal feature extractor (Sentinel-2 + Sentinel-1 RTC + NASADEM) to estimate per-pixel walking friction, blends it with classical OSM road-graph routing, runs multi-source shortest paths from healthcare facilities, and reports **calibrated** per-pixel travel-time bounds via split-conformal prediction. Validated against MAP 2020 (Weiss et al., *Nature Medicine*) at Spearman ρ = 0.674 with MAE 2.89 min in Blantyre, and at country scale on Sierra Leone (13M pixels, 100m). **No fine-tune.** TerraMind is used here in a way that isn't a downstream task head — it's the friction-surface backbone for a graph algorithm.

---

## Why this is "beyond just another fine-tune"

The TerraMind paper and the existing PANGAEA-style demos use it as a feature backbone for *classification / segmentation* fine-tunes (LULC, water bodies, change detection). Tirtha uses TerraMind embeddings as **the cost layer of a shortest-path graph**. Specifically:

1. Fetch a 224×224 multimodal chip (12 S2L2A bands + 2 S1 RTC bands in dB + NASADEM, all v1-normalized).
...
5. Run multi-source Dijkstra on the friction raster from every OSM-tagged healthcare facility → walking-time-to-nearest-care raster.
6. **Calibrate** the uncertainty: bootstrap the probe B=200×, ensemble K=40 friction surfaces, propagate through MCP, then apply split-conformal CQR using MAP 2020 as proxy reference.

The result is a per-pixel walking-time raster *with calibrated confidence intervals* — the first such artifact for healthcare accessibility that I'm aware of.
...
Expected Calibration Error: 0.484 → **0.003** (177× improvement). The honest 95% CI half-width is ±6 min. MAP 2020 has no uncertainty layer; AccessMod has none; Wu et al. 2025 has none. Tirtha does.
...
Three honest questions for the team:

1. **Is using TerraMind as the cost layer of a shortest-path graph a Blue-Sky-style application** (vs the segmentation / detection demos that have won previous rounds)?
2. **Does the calibrated-uncertainty contribution land** — split-conformal CQR against MAP 2020 as proxy reference, or is there a more defensible calibration target you'd recommend?
3. **The DHS supervision loop** (calibrating against household-reported travel times from the DHS Program) is the natural next step but blocked on weeks of registration paperwork. Is there an IBM/ESA partner you'd suggest who has already done that registration and might be open to a co-authored validation?

...
In dense cities — Manhattan, central London, central Tokyo — physical access to healthcare is taken for granted. You can walk to a clinic; if you can't walk, you can call a taxi.
...
The headline number is **Spearman ρ = 0.674 vs the published MAP 2020 raster**, with MAE 2.89 minutes and a small positive bias (we're slightly more pessimistic than MAP, which makes sense because our 10m grid finds longer detours that MAP's ~925m grid bilinear-interpolates over).
